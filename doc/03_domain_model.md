...
- `TerrainMap`의 호수 칸은 진입 가능한 정적 지형으로 본다. 자라가 없는 호수 위에서의 추락 실패는 `Stage`가 판정한다.
...
| TerrainMap | 스테이지를 구성하는 정적 지형 맵. 호수 칸은 진입 가능 지형이며 추락 여부는 Stage가 판정 |
| TerrainType | 육지, 호수, 안전지대, 벽, 시작 지점, 목적지 등 지형 종류 |
